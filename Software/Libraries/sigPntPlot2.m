function sigRange = sigPntPlot2(x,y,posSigIdx,negSigIdx,poscolor,negcolor)
sigRange = nan(1,6);
interval = x(2) - x(1); 
if ~isempty(y) 
%     posSigIdx = y > threshH;
        if ~isempty(find(posSigIdx,1))
            ...
            poscolor=poscolor/255;
            if posContTag
                
                plot(x(possIdx:poseIdx),y(possIdx:poseIdx),'-','Color',poscolor,'LineWidth',3);
%                 plot(x(possIdx:poseIdx),y(possIdx:poseIdx),'-','Color',[1,0,0],'LineWidth',3);
                ...
                plot(sigPX,sigPmY,'.','Color',poscolor,'LineWidth',3);
            end
%             disp('Activation')
        end
%         negSigIdx = y < threshL;
        if ~isempty(find(negSigIdx,1))
            ...
            negcolor=negcolor/255;
            if negContTag
                plot(x(negsIdx:negeIdx),y(negsIdx:negeIdx),'-','Color',negcolor,'LineWidth',3);
%                 plot(x(negsIdx:negeIdx),y(negsIdx:negeIdx),'-','Color',[0,0,1],'LineWidth',3);
                ...
                plot(sigNX,sigNmY,'.','Color',negcolor,'LineWidth',3);
